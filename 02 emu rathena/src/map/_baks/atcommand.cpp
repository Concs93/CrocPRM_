...
// @myclone [dur_ms=3000] [friendly=1] [copy_equip=1] [skill_id=0] [skill_lv=0]
ACMD_FUNC(myclone)
{
	if (sd->bl.type != BL_PC) return -1;
	if (!sd) return -1;
	...
	int skill_lv     = 0;
	int biolab_i     = 0;
	int fx_id        = CLONE_BIOLAB_EFFECT_ID;
	int copy_pskills = 0;
	int give_class_kit = 0;
	unsigned int hp_fixed = 0;

	// --- KV mode ---
	bool kv_mode = (message && strchr(message,'=') != nullptr);
	int  instant = 0;
	int  random_spawn = 1; // Default: 1 square de distância

	// FX extras
	int fx2 = 0, fx3 = 0, fx4 = 0, fx5 = 0;

	// Help
	if (message && *message == '?') {
		clif_displaymessage(fd,"Uso: @myclone <dur_ms> <friendly 1|0> <copy_eq 1|0> <skill_id> <skill_lv> <biolab 1|0> <fx_id> <copy_pskills 1|0> <give_class_kit 1|0> <hp_fixed> [fx2 fx3 fx4 fx5]");
		clif_displaymessage(fd,"KV mode: @myclone dur=3000 friendly=1 copy_eq=1 skill=2002/10 biolab=0 fx=744 fx2=586 fx3=865 random_spawn=1");
		return 0;
	}

	// Parsing
	if (kv_mode) {
		char buf[512]; safestrncpy(buf, message, sizeof(buf));
		for (char* tok = strtok(buf, " "); tok; tok = strtok(nullptr, " ")) {
			char* eq = strchr(tok, '=');
			if (!eq) continue;
			*eq = '\0';  // CORREÇÃO: usar \0 em vez de \\0
			const char* key = tok;
			const char* val = eq + 1;

			if (!strcasecmp(key, "dur") || !strcasecmp(key, "duration")) {
				dur_ms = atoi(val); if (dur_ms < 1) dur_ms = 1;
			} else if (!strcasecmp(key, "friendly"))  friendly = atoi(val) ? 1 : 0;
			else if (!strcasecmp(key, "copy_eq"))     copy_eq = atoi(val) ? 1 : 0;
			else if (!strcasecmp(key, "pskills"))     copy_pskills = atoi(val) ? 1 : 0;
			else if (!strcasecmp(key, "kit"))         give_class_kit = atoi(val) ? 1 : 0;
			else if (!strcasecmp(key, "biolab"))      biolab_i = atoi(val) ? 1 : 0;
			else if (!strcasecmp(key, "fx"))          fx_id = atoi(val);
			else if (!strcasecmp(key, "fx2"))         fx2 = atoi(val);
			else if (!strcasecmp(key, "fx3"))         fx3 = atoi(val);
			else if (!strcasecmp(key, "fx4"))         fx4 = atoi(val);
			else if (!strcasecmp(key, "fx5"))         fx5 = atoi(val);
			else if (!strcasecmp(key, "hp") || !strcasecmp(key, "hp_fixed")) {
				int tmp = atoi(val); hp_fixed = (tmp > 0) ? (unsigned)tmp : 0;
			} else if (!strcasecmp(key, "instant"))   instant = atoi(val) ? 1 : 0;
			else if (!strcasecmp(key, "random_spawn")) random_spawn = atoi(val);
			else if (!strcasecmp(key, "skill")) {
				skill_id = atoi(val);
				const char* slash = strchr(val, '/');
				if (slash) { skill_lv = atoi(slash + 1); if (skill_lv < 1) skill_lv = 1; }
			}
		}
	} else if (message && *message) {
		int read = sscanf(message, "%d %d %d %d %d %d %d %d %d %u %d %d %d %d",
			&dur_ms, &friendly, &copy_eq, &skill_id, &skill_lv,
			&biolab_i, &fx_id, &copy_pskills, &give_class_kit, &hp_fixed,
			&fx2, &fx3, &fx4, &fx5);
		if (read >= 1)  { if (dur_ms < 1) dur_ms = 1; if (dur_ms > 600000) dur_ms = 600000; }
		if (read >= 2)  friendly       = friendly ? 1 : 0;
		if (read >= 3)  copy_eq        = copy_eq ? 1 : 0;
		if (read >= 4)  { if (skill_id < 0) skill_id = 0; }
		if (read >= 5)  { if (skill_lv < 0) skill_lv = 0; }
		if (read >= 6)  biolab_i       = biolab_i ? 1 : 0;
		if (read >= 7)  { if (fx_id <= 0) fx_id = CLONE_BIOLAB_EFFECT_ID; }
		if (read >= 8)  copy_pskills   = copy_pskills ? 1 : 0;
		if (read >= 9)  give_class_kit = give_class_kit ? 1 : 0;
	}

	// -------- Flags do clone --------
	uint32 flags = CLONE_COPY_BASE | CLONE_COPY_ALL_STATS | CLONE_AI_CAST_ENABLED;
	if (copy_eq)        flags |= CLONE_COPY_EQUIP_ATKDEF;
	if (copy_pskills)   flags |= CLONE_COPY_PLAYER_SKILLS;
	if (give_class_kit) flags |= CLONE_GIVE_CLASS_SKILLS;
	if (biolab_i)       flags |= CLONE_BIOLAB_FX;
	
	// Debug para verificar a duração
	ShowDebug("[Clone Debug] @myclone dur_ms=%d, friendly=%d, copy_eq=%d, skill_id=%d, skill_lv=%d\n", 
		dur_ms, friendly, copy_eq, skill_id, skill_lv);
	ShowDebug("[Clone Debug] @myclone message='%s', kv_mode=%d\n", message ? message : "NULL", kv_mode);

	// Spawn-cast quando há extra skill, ou quando pedirem instant=1 no KV-mode
	// (marcador para MSC_SPAWN via mobskill_event após o spawn)
	// ATENÇÃO: se não houver skill extra, esse flag é inofensivo.
	//          mas mantemos ligado apenas quando faz sentido.
	//          Isso facilita o caminho natural de cast no spawn.
	// Setaremos também MST_TARGET na entrada da extra.
	// Validação de skill extra
	s_clone_skill_entry extra[1];
	int extra_count = 0;
	if (skill_id > 0 && skill_lv > 0) {
		extra[0].skill_id = (uint16)skill_id;
		extra[0].skill_lv = (uint16)skill_lv;
		extra[0].chance_permyriad = 10000;
		extra[0].casttime_ms = 0;
		extra[0].delay_ms    = 0;
		extra[0].target_flags = 0;         // alvo hostil padrão
		flags |= CLONE_ALLOW_EXTRA_SKILLS;
		extra_count = 1;
	}
	...
	int fx_buf[5];
	int fx_count = 0;
	auto push_fx = [&](int v){ if (v > 0 && fx_count < 5) fx_buf[fx_count++] = v; };
	push_fx(fx_id); push_fx(fx2); push_fx(fx3); push_fx(fx4); push_fx(fx5);

	// mode bits
	int mode_bits = 0;
	#ifdef MD_AGGRESSIVE
		mode_bits |= MD_AGGRESSIVE;
	#endif
	#ifdef MD_CANMOVE
		mode_bits |= MD_CANMOVE;
	#endif
	#ifdef MD_CANATTACK
		mode_bits |= MD_CANATTACK;
	#endif
	#ifdef MD_ANGRY
		mode_bits |= MD_ANGRY;
	#endif
	#ifdef MD_CANUSESKILL
		mode_bits |= MD_CANUSESKILL;
	#endif

	static std::unordered_map<int,int> myclone_icd_ms;
	int now = (int)gettick();
	int last = myclone_icd_ms[sd->bl.id];
	const int ICD = 800;
	if (now - last < ICD) {
		clif_displaymessage(fd, "[myclone] bloqueado por ICD.");
		return true;
	}
	myclone_icd_ms[sd->bl.id] = now;

	const int gid = mob_clone_spawn_ext(
		sd, sd->bl.m, sd->bl.x, sd->bl.y,
		"", sd->bl.id, mode_bits, (friendly != 0),
		flags,
		extra_count ? extra : nullptr, extra_count,
		fx_id,
		hp_fixed,
		fx_count ? fx_buf : nullptr, fx_count,
		random_spawn
	);

	if (gid > 0) {
		pc_setaccountreg(sd, add_str("last_clone_gid"), gid);
		char buf[224];
		snprintf(buf, sizeof(buf),
			"[myclone] gid=%d dur=%dms friendly=%d copy_eq=%d skill=%d/%d biolab=%d fx=%d copy_pskills=%d kit=%d hp_fixed=%u extras=%d random_spawn=%d",
			gid, dur_ms, friendly, copy_eq, skill_id, skill_lv, biolab_i, fx_id, copy_pskills, give_class_kit, hp_fixed, fx_count, random_spawn);
		clif_displaymessage(fd, "Clone criado com sucesso.");
		clif_displaymessage(fd, buf);
		if (dur_ms > 0) {
			if (TBL_MOB* md2 = map_id2md(gid)) {
				md2->deletetimer = add_timer(gettick() + dur_ms, mob_timer_delete, gid, 0);
			}
		}
	} else {
		clif_displaymessage(fd, "Falha ao criar clone.");
	}
