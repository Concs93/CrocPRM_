...
static int clone_ai_dbg_timer(int tid, int64 tick, int id, intptr_t data) {
    ...
    ShowInfo("[clone-ai] id=%d mode=%u {A%d M%d K%d S%d} target=%d ai=%d sc=%d\n",
        id, (unsigned)md->status.mode, a,m,k,s, md->target_id, (int)md->special_state.ai, (int)has_sc
    );
    return 0;
}
...
int mobskill_use(struct mob_data *md, t_tick tick, int event)
{
	...
	for (n = 0; n < ms.size(); i++, n++) {
		...
		//Execute skill
		// Para clones: respeita target fixo ou procura novo
		if (md->special_state.clone) {
			// Se o target é fixo do master, mantém ele sempre
			if (md->special_state.clone_target_fixed && md->target_id > 0) {
				struct block_list *current_target = map_id2bl(md->target_id);
				if (current_target && current_target->m == md->bl.m) {
					// Target fixo ainda é válido, mantém ele (sem limite de distância)
				} else {
					// Target fixo inválido, procura novo
					md->special_state.clone_target_fixed = 0;
					md->special_state.clone_target_locked = 0;
				}
			}
			
			// Se não tem target fixo ou target inválido, procura novo
			if (!md->special_state.clone_target_fixed && !md->special_state.clone_target_locked) {
...
int mob_clone_spawn_ext(
  ...
  #define MD_CANUSESKILL (e_mode)0x0008
	...
	if (friendly) {
		// Aliado: primeiro tenta herdar o target do master
		if (sd->ud.target) {
			struct block_list *master_target = map_id2bl(sd->ud.target);
			if (master_target && master_target->m == md->bl.m) {
				// Master tem target válido, clone herda ele (sem limite de distância)
				md->target_id = master_target->id;
				md->special_state.clone_target_locked = 1;
				md->special_state.clone_target_fixed = 1; // Target fixo do master
			} else {
				// Target do master inválido, procura inimigo próximo do clone
				struct block_list *enemy = battle_getenemy(&md->bl, DEFAULT_ENEMY_TYPE(md), 10);
				if (enemy && enemy->id > 0) {
					md->target_id = enemy->id;
					md->special_state.clone_target_locked = 1;
				} else {
					// Se não encontrar inimigo próximo, procura qualquer mob no mapa
					enemy = battle_getenemy(&md->bl, BL_MOB, 25);
					if (enemy && enemy->id > 0) {
						md->target_id = enemy->id;
						md->special_state.clone_target_locked = 1;
					}
			...
			// Master não tem target, procura inimigo próximo do clone
			struct block_list *enemy = battle_getenemy(&md->bl, DEFAULT_ENEMY_TYPE(md), 10);
			if (enemy && enemy->id > 0) {
				md->target_id = enemy->id;
				md->special_state.clone_target_locked = 1;
	...
	if (friendly) {
		md->master_id = master_id;
		md->special_state.ai = AI_ATTACK; // ataca inimigos do dono

		// (opcional) herdar alvo atual do player, se existir na sua base
		#ifdef HAVE_UD_TARGET
		if (sd->ud.target) {
			if (auto t = map_id2bl(sd->ud.target))
				md->target_id = t->id; // só sugere; IA decide
		}
		#endif
	} else {
		md->master_id = 0;                 // hostil: desvinculado do dono
		md->special_state.ai = AI_ATTACK;  // IA de ataque ligada
