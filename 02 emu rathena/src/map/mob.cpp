...
#include <memory>
#include <string>
#include <climits> // se ainda não tiver
...
const int fx_id = CLONE_BIOLAB_EFFECT_ID;
...
// ---- DEBUG: imprime a IA do clone 1s após o spawn ----
static int clone_ai_dbg_timer(int tid, int64 tick, int id, intptr_t data) {
    TBL_MOB* md = map_id2md(id);
    if (!md) return 0;
	md->status.mode = (e_mode)(md->status.mode & ~MD_NOCAST);

    int a=0,m=0,k=0,s=0;
#ifdef MD_AGGRESSIVE
    a = (md->status.mode & MD_AGGRESSIVE)   != 0;
#endif
#ifdef MD_CANMOVE
    m = (md->status.mode & MD_CANMOVE)      != 0;
#endif
#ifdef MD_CANATTACK
    k = (md->status.mode & MD_CANATTACK)    != 0;
#endif
#ifdef MD_CANUSESKILL
    s = (md->status.mode & MD_CANUSESKILL)  != 0;
#endif

    // (opcional) mostrar se há SC_MODECHANGE aplicado
    bool has_sc = false;
#ifdef SC_MODECHANGE
    if (status_change* sc = status_get_sc(&md->bl)) {
        has_sc = sc->data[SC_MODECHANGE] != nullptr;
    }
#endif
...
static TIMER_FUNC(clone_biolab_fx_timer) {
    struct block_list* bl = map_id2bl(id);
    if (!bl) return 0;
    int fx_id = (int)data;
    if (fx_id <= 0) fx_id = CLONE_BIOLAB_EFFECT_ID;
    clif_specialeffect(bl, fx_id, AREA);
    add_timer(tick + 3000, clone_biolab_fx_timer, id, data);
    return 0;
}
...
int mob_spawn (struct mob_data *md)
{
	...
	// Para clones com target fixo, preserva o target_id
	if (!md->special_state.clone || !md->special_state.clone_target_fixed) {
	md->target_id = 0;
	}
...
static bool mob_ai_sub_hard(struct mob_data *md, t_tick tick)
{
	...
	md->last_thinktime = tick;
		...
		// AI customizada para clones - mantém target fixo
		if (md->special_state.ai == AI_CLONE) {
			// Clones só atacam se tiverem target válido
			if (md->target_id > 0) {
				struct block_list *target = map_id2bl(md->target_id);
				if (target && target->m == md->bl.m) {
					// Target válido, verifica se pode atacar
					if (md->ud.attacktimer == INVALID_TIMER && md->ud.canact_tick <= tick) {
						// Pode atacar agora - executa ataque
						if (battle_check_range(&md->bl, target, md->status.rhw.range)) {
							// No alcance - ataca
							unit_attack(&md->bl, target->id, 1);
							return true;
						} else {
							// Fora de alcance - persegue
							if (unit_can_move(&md->bl)) {
								unit_walktobl(&md->bl, target, 1, 1);
							}
							return true;
						}
					}
				} else {
					// Target inválido, procura novo
					if (!md->special_state.clone_target_fixed) {
						struct block_list *nearest_enemy = battle_getenemy(&md->bl, DEFAULT_ENEMY_TYPE(md), 10);
						if (nearest_enemy && nearest_enemy->id > 0) {
							md->target_id = nearest_enemy->id;
						}
					}
				}
			} else {
				// Sem target, procura um APENAS se não for fixo
				if (!md->special_state.clone_target_fixed) {
					struct block_list *nearest_enemy = battle_getenemy(&md->bl, DEFAULT_ENEMY_TYPE(md), 10);
					if (nearest_enemy && nearest_enemy->id > 0) {
						md->target_id = nearest_enemy->id;
			...
			// Clone sempre retorna true para manter atividade
			return true;
		}
...
using mob_skill_ptr = std::shared_ptr<s_mob_skill>;
using mob_skill_vec = std::vector<mob_skill_ptr>;

// Constrói e empurra uma entrada de skill de mob
//static void clone_add_mobskill(mob_skill_vec& vec, uint16 id, uint16 lv,
//                               uint16 chance_permyriad, uint32 cast_ms, uint32 delay_ms,
//                               uint16 target_flags)
static void clone_add_mobskill(mob_skill_vec& vec, uint16 id, uint16 lv,
                               uint16 chance_permyriad, uint32 cast_ms, uint32 delay_ms,
                               uint16 target_flags, uint32 flags)
{
	auto ms = std::make_shared<s_mob_skill>();
	ms->skill_id = id;
	ms->skill_lv = lv ? lv : 1;
	ms->permillage = chance_permyriad ? chance_permyriad : 800; // 0..10000
	ms->casttime = cast_ms;
	ms->delay    = delay_ms;
	ms->state    = MSS_ANY;
	//ms->hp_rate    = 100;   // usa até 100% de HP
	//ms->sp_rate    = 0;     // sem exigência de SP
	//ms->attack_rate= 0;     // não exige estar "under attack"
	ms->emotion  = -1;
	ms->target   = target_flags ? target_flags : MST_TARGET;
	ms->cond1    = -1;
	ms->cond2    = -1;

	if (flags & CLONE_FORCE_SPAWN_CAST) {
		ms->cond1 = MSC_SPAWN;
		if (ms->casttime < 0) ms->casttime = 0;
		if (ms->delay    < 0) ms->delay    = 0;
		if (ms->permillage < 1) ms->permillage = 10000;
	} else {
		ms->state = MSS_ANY;
	}
...
// Copia skills aprendidas do player
static void clone_copy_player_skills(mob_skill_vec& dst, struct map_session_data* sd) {
    if (!sd) return;

    for (uint16 sk = 1; sk < MAX_SKILL; ++sk) {
        // EVITA chamar pc_checkskill() para skills que não existem no seu skill_db
        if (skill_db.find(sk) == nullptr)
            continue;

        int lv = pc_checkskill(sd, sk);
        if (lv > 0)
            clone_add_mobskill(dst, sk, (uint16)lv, 1200, 0, 2000, 0, 0);
    }
}

// Kit básico por classe (com validação no skill_db)
static void clone_give_class_kit(mob_skill_vec& dst, TBL_PC* sd)
{
    if (!sd) return;

    // Adiciona a skill ao vetor do clone SOMENTE se a skill existir no DB
    auto safe_add = [&](uint16 id, uint16 lv,
                        uint16 chance_permyriad, uint32 casttime_ms,
                        uint32 delay_ms, uint16 target_flags)
    {
        // rAthena “novo”:
        if (/* skill_db.find(id) */ (skill_db.find(id) == nullptr)) // <-- use esta linha
            return;

        // rAthena “antigo” (se preferir): 
        // if (!skill_get_db(id)) return;

        clone_add_mobskill(dst, id, lv, chance_permyriad, casttime_ms, delay_ms, target_flags, 0);
    };

    const int job = sd->status.class_;
    switch (job) {
        case JOB_SWORDMAN:
            // Bash lv10 (15% por tentativa), Provoke lv10 (8%)
            safe_add(  5 /* Bash        */, 10, 1500, 0, 1500, 0);
            safe_add(  6 /* Provoke     */, 10,  800, 0, 3000, 0);
            break;

        case JOB_THIEF:
            // Envenom lv5 (9%)
            safe_add( 47 /* Envenom     */,  5,  900, 0, 2500, 0);
            break;

        default:
            // Magnum Break lv10 (10%), Cold Bolt lv5 (10%)
            safe_add( 89 /* MagnumBreak */, 10, 1000, 0, 2000, 0);
            safe_add( 34 /* Cold Bolt   */,  5, 1000, 0, 2500, 0);
            break;
    }
}
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	if( !rebirth ) {
		...
		if( md->vd &&  pcdb_checkid(md->vd->class_) ) {//Player mobs are not removed automatically by the client.
...
int mobskill_use(struct mob_data *md, t_tick tick, int event)
{
	...
	// Para clones: só permite 1 cast por vida
	if (md->special_state.clone && md->special_state.clone_casted) {
		return 0;
	}
	...
	for (n = 0; n < ms.size(); i++, n++) {
		...
		if (ms[i]->cond1 == event) {
			...
			if (md->special_state.clone && event == MSC_SPAWN) {
			}
		}
		...
		//Execute skill
		// Para clones: força uso do target definido
		if (md->special_state.ai == AI_CLONE) {
			// Clones sempre usam MST_TARGET para respeitar md->target_id
			skill_target = MST_TARGET;
		} else {
		...
		// Para clones: marca que já castou uma skill
		if (md->special_state.clone) {
			md->special_state.clone_casted = 1;
		}
...
int mobskill_event(struct mob_data *md, struct block_list *src, t_tick tick, int flag)
{
	...
	// Clone spawn event - handled elsewhere

	target_id = md->target_id;
	// Para clones com target fixo, não altera o target
	if (!md->special_state.clone || !md->special_state.clone_target_fixed) {
...
int mob_clone_spawn(struct map_session_data *sd, int16 m, int16 x, int16 y, const char *event, int master_id, enum e_mode mode, int flag, unsigned int duration)
{
	...
	if( tree != nullptr && !tree->skills.empty() ){
		...
		for (const auto &it : skill_list) {
			...
			ms->state = MSS_ANY;
			ms->permillage = 10000; // 100% chance para clones castarem imediatamente
			ms->emotion = -1;
			ms->cancel = 0;
			ms->casttime = skill_castfix(&sd->bl,skill_id, ms->skill_lv);
			ms->delay = 100+skill_delayfix(&sd->bl,skill_id, ms->skill_lv); // Delay mínimo para cast imediato
			ms->msg_id = 0;

			inf = skill_get_inf(skill_id);
			if (inf&INF_ATTACK_SKILL) {
				ms->target = MST_TARGET;
				ms->cond1 = MSC_SPAWN; // Tenta castar imediatamente no spawn
	...
	if (!md) return 0;
	md->status.mode = (e_mode)(md->status.mode & ~MD_NOCAST); //Failed?

	md->special_state.clone = 1;

	if (master_id || flag || duration) { //Further manipulate crafted char.
		md->special_state.ai = AI_ATTACK; // liga AI sempre
		if (!(flag&1) && master_id) {
			md->target_id = master_id;      // já mira no master
		}
...
int mob_clone_spawn_ext(
    TBL_PC* sd, int m, int x, int y,
    const char* death_event,
    int master_id, int mode, bool friendly,
    uint32 flags,
    const s_clone_skill_entry* extra_skills, int extra_count,
    int biolab_fx_id,
    unsigned int fixed_hp,           // 0 = não forçar
    const int* fx_ids, int fx_count, // lista de FX extras (opcional)
    int random_spawn,                // 0 = spawn exato, >0 = distância aleatória
    int target_id                    // 0 = auto, >0 = target específico
){
    nullpo_ret(sd);

    // 1) Calcula posição de spawn (aleatória se random_spawn > 0)
    int spawn_x = x, spawn_y = y;
    if (random_spawn > 0) {
        // Tenta encontrar uma posição válida em um raio de random_spawn squares
        int attempts = 0;
        const int max_attempts = 20;
        
        while (attempts < max_attempts) {
            // Gera posição aleatória em um quadrado de (random_spawn*2+1) x (random_spawn*2+1)
            int offset_x = (rnd() % (random_spawn * 2 + 1)) - random_spawn;
            int offset_y = (rnd() % (random_spawn * 2 + 1)) - random_spawn;
            
            int new_x = x + offset_x;
            int new_y = y + offset_y;
            
            // Verifica se a posição é válida (não é a mesma do master e é passável)
            if ((new_x != x || new_y != y) && 
                map_getcell(m, new_x, new_y, CELL_CHKPASS)) {
                spawn_x = new_x;
                spawn_y = new_y;
                break;
            }
        ...
        // Se não encontrou posição válida, usa a posição original
    }

    // 2) Pega um ID livre para clone
    int mob_id;
    ARR_FIND(MOB_CLONE_START, MOB_CLONE_END, mob_id, !mob_db.exists(mob_id));
    if (mob_id >= MOB_CLONE_END)
        return 0;

    // 2) DB temporário do clone
    auto db = std::make_shared<s_mob_db>();
    mob_db.put(mob_id, db);

    std::string base_name = sd->status.name ? sd->status.name : "Clone";

	// 3) Modo/IA no DB (precisa vir no DB pra nascer já agressivo)
	db->status.mode = (e_mode)(
		MD_AGGRESSIVE
		| MD_CANMOVE
		| MD_CANATTACK
		| MD_ANGRY
    ...
    db->range2 = 15; // chase / search - aumentado para clones encontrarem inimigos mais facilmente
    db->range3 = 15;
    db->sprite = "PORING"; // qualquer sprite; o visual real vem do viewdata do player
    db->name   = (flags & CLONE_NAME_AS_OWNER) ? (base_name + " Clone") : base_name;
    db->jname  = db->name;

    // 2.1) Status base copiados do player
    db->lv = (std::max)(1, status_get_lv(&sd->bl));
    memcpy(&db->status, &sd->base_status, sizeof(status_data));
    db->status.rhw      = sd->base_status.rhw;
    db->status.lhw      = sd->base_status.lhw;
    db->status.rhw.atk2 = sd->base_status.rhw.atk2;
    db->status.lhw.atk2 = sd->base_status.lhw.atk2;

#ifdef EF_NEUTRAL
    db->status.element = sd->base_status.element;
#endif
#ifdef RC_FORMLESS
    db->status.race = sd->base_status.race;
#endif
#ifdef SZ_SMALL
    db->status.size = sd->base_status.size;
#endif
#ifdef status_get_adelay
    db->status.adelay = status_get_adelay(&sd->bl);
#endif
#ifdef status_get_aspd_rate
    db->status.aspd_rate = status_get_aspd_rate(&sd->bl);
#endif

    db->status.hit   = sd->base_status.hit;
    db->status.flee  = sd->base_status.flee;
    db->status.cri   = sd->base_status.cri;
    db->status.def   = sd->base_status.def;
    db->status.mdef  = sd->base_status.mdef;
    db->status.def2  = sd->base_status.def2;
    db->status.mdef2 = sd->base_status.mdef2;
    db->status.hp    = sd->base_status.hp;    db->status.max_hp = sd->base_status.max_hp;
    db->status.sp    = sd->base_status.sp;    db->status.max_sp = sd->base_status.max_sp;

    // 2.2) Opcional: usar status "vivos" (equip/buffs)
    if (flags & CLONE_COPY_EQUIP_ATKDEF) {
        const status_data& bs = sd->battle_status;
        db->status.rhw.atk   = bs.rhw.atk;
        db->status.rhw.atk2  = bs.rhw.atk2;
        db->status.lhw.atk   = bs.lhw.atk;
        db->status.lhw.atk2  = bs.lhw.atk2;
        db->status.matk_min  = bs.matk_min;
        db->status.matk_max  = bs.matk_max;
        db->status.rhw.range = bs.rhw.range;
        db->status.def   = bs.def;
        db->status.mdef  = bs.mdef;
        db->status.def2  = bs.def2;
        db->status.mdef2 = bs.mdef2;
        db->status.hit   = bs.hit;
        db->status.flee  = bs.flee;
        db->status.flee2 = bs.flee2;
        db->status.cri   = bs.cri;
    }

    // 3) Modo/IA no DB (traga o 'mode' do caller e preserve os defaults já setados)
    db->status.mode = static_cast<e_mode>(db->status.mode | mode);
#ifndef MD_AGGRESSIVE
  #define MD_AGGRESSIVE  (e_mode)0x0001
#endif
#ifndef MD_CANMOVE
  #define MD_CANMOVE     (e_mode)0x0002
#endif
#ifndef MD_CANATTACK
  #define MD_CANATTACK   (e_mode)0x0004
#endif
#ifndef MD_CANUSESKILL
  #define MD_CANUSESKILL (e_mode)0x0008
#endif
    db->status.mode = static_cast<e_mode>(
        db->status.mode
      | MD_AGGRESSIVE
      | MD_CANMOVE
      | MD_CANATTACK
	...
	if (fixed_hp > 0) {
		db->status.max_hp = fixed_hp;
		db->status.hp     = fixed_hp;
	}

    // 4) Mob skills
    db->skill.clear();
    if (flags & CLONE_COPY_PLAYER_SKILLS)
        clone_copy_player_skills(db->skill, sd);
    if (flags & CLONE_GIVE_CLASS_SKILLS)
        clone_give_class_kit(db->skill, sd);
    if ((flags & CLONE_ALLOW_EXTRA_SKILLS) && extra_skills && extra_count > 0) {
        for (int i = 0; i < extra_count; ++i) {
            const auto& e = extra_skills[i];
            clone_add_mobskill(db->skill, e.skill_id, e.skill_lv, e.chance_permyriad,
                               e.casttime_ms, e.delay_ms, e.target_flags, flags);
        }
    }

	// 5) cria a instância
	// Evento de morte personalizado para clones com efeitos visuais
	std::string clone_death_event = "CloneDeathFX::OnCloneDeath";
	if (death_event && *death_event) {
		clone_death_event = std::string(death_event) + "::OnCloneDeath";
	}
	
	TBL_MOB* md = mob_once_spawn_sub(nullptr, (int16)m, (int16)spawn_x, (int16)spawn_y,
									db->name.c_str(), mob_id,
									clone_death_event.c_str(),
									(unsigned int)SZ_SMALL, (mob_ai)0);
	if (!md) return 0;
	md->status.mode = (e_mode)(md->status.mode & ~MD_NOCAST);

	// 6) marcações do clone (ALIADO x HOSTIL)
	md->special_state.clone = 1;
	md->state.aggressive = 1;
	
	// Para clones: define target imediatamente para evitar random walk
	if (friendly) {
		// SOLUÇÃO HÍBRIDA: Tenta battle_getenemy com ranges crescentes
		struct block_list *nearest_enemy = NULL;
		int ranges[] = {3, 5, 8, 12, 15, 20, 25};
		int range_count = sizeof(ranges) / sizeof(ranges[0]);
		
		// Tenta ranges crescentes para encontrar o mais próximo
		for (int i = 0; i < range_count; i++) {
			nearest_enemy = battle_getenemy(&sd->bl, BL_MOB, ranges[i]);
			if (nearest_enemy) {
				break;
			}
		}
		
		if (nearest_enemy && nearest_enemy->m == md->bl.m) {
			md->target_id = nearest_enemy->id;
			md->special_state.clone_target_locked = 1;
			md->special_state.clone_target_fixed = 1;
		}
	} else {
		// Hostil: mira no player
		md->target_id = sd->bl.id;
		md->special_state.clone_target_locked = 1;
	}
	
	// Para clones: desabilita random walk para evitar movimento desnecessário
	md->status.mode = (e_mode)(md->status.mode | MD_NORANDOMWALK);
	
	// Para clones: agenda verificação de target após pequeno delay
	// Isso permite que o master tenha tempo de definir o target após o cast
	if (friendly && md->special_state.clone) {
		add_timer(gettick() + 500, [](int tid, int64 tick, int id, intptr_t) {
			TBL_MOB* md = map_id2md(id);
			if (!md || !md->special_state.clone) return 0;
			
			// Se ainda não tem target, tenta herdar do master
			if (!md->target_id) {
				TBL_PC* sd = map_id2sd(md->master_id);
				if (sd && sd->ud.target) {
					struct block_list *master_target = map_id2bl(sd->ud.target);
							if (master_target && master_target->m == md->bl.m) {
								md->target_id = master_target->id;
								md->special_state.clone_target_locked = 1;
								md->special_state.clone_target_fixed = 1;
							}
				}
	...
	// Debug para verificar o mode do DB
	
	// Garante que o clone não tenha skilltimer ativo e possa agir imediatamente
	md->ud.skilltimer = INVALID_TIMER;
	md->ud.canact_tick = 0;

	// Bits de IA em runtime (reforça o que veio do DB)
	// Para clones, preserva o mode do DB (que já tem os bits corretos)
	if (md->special_state.clone) {
		// Clone já tem o mode correto do DB, não sobrescreve
	} else {
		// Para mobs normais, aplica os bits padrão
	md->status.mode = (e_mode)(
		md->status.mode
	#ifdef MD_AGGRESSIVE
		| MD_AGGRESSIVE
	#endif
...
#ifdef MD_CANUSESKILL
	md->status.mode = (e_mode)(md->status.mode | MD_CANUSESKILL);
#endif
	md->status.mode = (e_mode)(md->status.mode & ~MD_NOCAST);
	}
	
	// Para clones: força evento MSC_SPAWN imediatamente para cast rápido
	if (md->special_state.clone) {
		mobskill_event(md, &md->bl, gettick(), MSC_SPAWN);
	}

	// Aliado mantém vínculo; Hostil NUNCA tem vínculo com o dono
	if (friendly) {
		md->master_id = master_id;
		md->special_state.ai = AI_CLONE; // AI customizada para clones

		// Target já foi definido na seção acima
	} else {
		md->master_id = 0;                 // hostil: desvinculado do dono
		md->special_state.ai = AI_CLONE;   // AI customizada para clones
		md->target_id = sd->bl.id; // só sugere; IA decide
	}

    // 7) Visual do player
    if (md->vd) {
        mob_set_dynamic_viewdata(md); // duplica vd e marca vd_changed
        md->vd->class_ = sd->vd.class_;
        md->vd->sex    = sd->vd.sex;
        md->vd->hair_style  = sd->vd.hair_style;
        md->vd->hair_color  = sd->vd.hair_color;
        md->vd->cloth_color = sd->vd.cloth_color;
        md->vd->weapon      = sd->vd.weapon;
        md->vd->shield      = sd->vd.shield;
        md->vd->head_top    = sd->vd.head_top;
        md->vd->head_mid    = sd->vd.head_mid;
        md->vd->head_bottom = sd->vd.head_bottom;
    #ifdef VIEWDATA_HAVE_ROBE
        md->vd->robe        = sd->vd.robe;
    #endif
    }

    // 8) Spawn (liga timers de AI)
    mob_spawn(md);

	
    // 8.1) Refresca MODECHANGE (pós-spawn, como o core costuma fazer)
	sc_start4(nullptr, &md->bl, SC_MODECHANGE, 100,
			1, 0,
			MD_AGGRESSIVE | MD_CANATTACK | MD_CANMOVE | MD_ANGRY,
			0, 60000);

    // 9) FXs
	if (md->vd) {
		if (flags & CLONE_BIOLAB_FX) {
			mob_set_dynamic_viewdata(md);
			if (biolab_fx_id <= 0) biolab_fx_id = CLONE_BIOLAB_EFFECT_ID;
			md->vd->cloth_color = (md->vd->cloth_color ? md->vd->cloth_color : 1);
			md->vd->hair_color  = (md->vd->hair_color  ? md->vd->hair_color  : 1);

			clif_specialeffect(&md->bl, biolab_fx_id, AREA);
			add_timer(gettick() + 3000, clone_biolab_fx_timer, md->bl.id, biolab_fx_id);
		}

		// FX extras (independente do “biolab”)
		if (fx_ids && fx_count > 0) {
			for (int i = 0; i < fx_count; ++i) {
				const int fx = fx_ids[i];
				if (fx <= 0) continue;
				clif_specialeffect(&md->bl, fx, AREA);
				add_timer(gettick() + 3000, clone_biolab_fx_timer, md->bl.id, fx);
			}
		}
	}

    // 10) Log de debug 1s depois
	add_timer(gettick() + 1000, clone_ai_dbg_timer, md->bl.id, 0);
