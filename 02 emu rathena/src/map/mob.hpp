...
// Efeito padrão (troque pelo ID que você gostar depois de testar com @effect)
#ifndef CLONE_BIOLAB_EFFECT_ID
#define CLONE_BIOLAB_EFFECT_ID  744 // exemplo; escolha o seu id favorito do @effect
#endif
...
/** Flags to control clone copy/cast behaviour. */
enum e_clone_copy : uint32 {
	CLONE_COPY_BASE           = 0x0001, // comportamento antigo: name/lv/base_status
	CLONE_COPY_ALL_STATS      = 0x0002, // aspd/hit/flee/crit/atk/matk/def/mdef/element/race/size
	CLONE_COPY_EQUIP_ATKDEF   = 0x0004, // (opcional) aplicar bônus vivos de equips
	CLONE_COPY_PLAYER_SKILLS  = 0x0008, // copiar skills aprendidas do player (pc_checkskill)
	CLONE_GIVE_CLASS_SKILLS   = 0x0010, // dar um "kit" básico por classe
	CLONE_ALLOW_EXTRA_SKILLS  = 0x0020, // aceitar skills extras por argumento
	CLONE_AI_CAST_ENABLED     = 0x0040, // liga flags de AI para conjurar
	CLONE_NAME_AS_OWNER       = 0x0080, // "<Nome> Clone"
	CLONE_FORCE_SPAWN_CAST    = 0x0100,  // <— NOVA: usa extra_skills no MSC_SPAWN (cast imediato)

	CLONE_BIOLAB_FX = 1u << 12, // brilho/transparência azul
};

/** Optional extra skills descriptor for clones. */
struct s_clone_skill_entry {
	uint16 skill_id;
	uint16 skill_lv;
	uint16 chance_permyriad; // 0..10000 (1000 = 10%)
	uint32 casttime_ms;      // 0 = sem cast extra
	uint32 delay_ms;         // delay entre conjurações dessa entrada
	uint16 target_flags;     // 0 = alvo hostil padrão (MST_TARGET)
};
...
struct s_mob_db {
	...
	uint16 damagetaken;
	bool biolab = false;   // padrão: desligado
...
struct mob_data {
	...
	struct s_specialState {
		...
		unsigned int clone_casted : 1;/* clone já castou skill? 1:0 */
		unsigned int clone_target_locked : 1;/* clone target já foi definido? 1:0 */
		unsigned int clone_target_fixed : 1;/* clone target é fixo do master? 1:0 */
...
// mantenha a declaração da versão "nova" (ext) — usada pelo @myclone
int mob_clone_spawn_ext(
    ...
    int biolab_fx_id,
    unsigned int fixed_hp,
    const int* fx_ids, int fx_count,
    int random_spawn = 0,
    int target_id = 0
);

// *** RESTAURAÇÃO: assinatura padrão (usada por @clone/@slaveclone/@evilclone) ***
int mob_clone_spawn(struct map_session_data *sd, int16 m, int16 x, int16 y,
                    const char *event, int master_id, enum e_mode mode,
                    int flag, unsigned int duration);
