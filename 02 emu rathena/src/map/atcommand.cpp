...
ACMD_FUNC(myclone)
{
	...
	int  target_id = 0; // Target específico para o clone
	...
	if (message && *message == '?') {
		...
		clif_displaymessage(fd,"KV mode: @myclone dur=3000 friendly=1 copy_eq=1 skill=2002/10 biolab=0 fx=744 fx2=586 fx3=865 random_spawn=1 target=12345");
		return 0;
	}
	...
	if (kv_mode) {
		...
		for (char* tok = strtok(buf, " "); tok; tok = strtok(nullptr, " ")) {
			...
			*eq = '\0';  // CORREÇÃO: usar \0 em vez de \\0
			...
			else if (!strcasecmp(key, "target"))     target_id = atoi(val);
