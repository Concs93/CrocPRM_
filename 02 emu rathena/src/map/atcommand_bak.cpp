...
ACMD_FUNC(myclone)
{
	...
	// -------- Defaults --------
	int dur_ms       = 15000;   // duração do clone
	int friendly     = 1;       // 1=aliado, 0=hostil
	int copy_eq      = 1;       // 1=copiar ATK/DEF vivos dos equips/buffs
	int skill_id     = 0;       // 0=sem skill extra
	int skill_lv     = 0;
	int biolab_i     = 0;       // 0=sem FX biolab, 1=com FX biolab
	int fx_id        = CLONE_BIOLAB_EFFECT_ID; // 744 por padrão
	int copy_pskills = 0;       // 1=copiar skills do player
	int give_class_kit = 0;     // 1=dar kit básico por classe
	unsigned int hp_fixed = 0;  // >0 = HP fixo

	// FX extras opcionais (até 4 além do principal)
	int fx2 = 0, fx3 = 0, fx4 = 0, fx5 = 0;

	// -------- Ajuda --------
	if (message && *message == '?') {
		clif_displaymessage(fd,
			"Uso: @myclone <dur_ms> <friendly 1|0> <copy_eq 1|0> <skill_id> <skill_lv> "
			"<biolab 1|0> <fx_id> <copy_pskills 1|0> <give_class_kit 1|0> <hp_fixed> [fx2 fx3 fx4 fx5]");
		clif_displaymessage(fd,
			"Ex1 (aliado, copia equips, sem FX): @myclone 15000 1 1 0 0 0 0 1 1 0");
		clif_displaymessage(fd,
			"Ex2 (hostil, copia equips, HP fixo 50k, com FX biolab 744): @myclone 20000 0 1 0 0 1 744 1 1 50000");
		clif_displaymessage(fd,
			"Ex3 (aliado, FX extras 411 421 422): @myclone 12000 1 1 0 0 0 0 1 1 0 411 421 422");
		return 0;
	}

	// -------- Parsing --------
	if (message && *message) {
		// Tenta ler até 14 valores.
		// Ordem:
		// dur_ms friendly copy_eq skill_id skill_lv biolab fx_id copy_pskills give_class_kit hp_fixed fx2 fx3 fx4 fx5
		...
		copy_eq        = copy_eq       ? 1 : 0;
		copy_pskills   = copy_pskills  ? 1 : 0;
		give_class_kit = give_class_kit? 1 : 0;

		// Clamps mínimos (só se veio algo)
		...
		// hp_fixed pode ser 0 (desligado) ou >0
	}

	// -------- Validação de skill extra --------
	if (skill_id > 0) {
#ifdef HAVE_SKILL_DB_FIND
		if (!skill_db.find((uint16)skill_id)) {
			clif_displaymessage(fd, "Skill extra inválida/indefinida no DB; ignorando.");
			skill_id = 0;
			skill_lv = 0;
		}
#else
		// Se sua base não tem skill_db.find, você pode trocar por outra verificação,
		// ou simplesmente confiar no ID vindo.
#endif
	}
	...
	snprintf(echo, sizeof(echo),
		"[myclone] dur=%dms friendly=%d copy_equip=%d copy_pskills=%d class_kit=%d hp_fixed=%u fx_main=%d",
		dur_ms, friendly, copy_eq, copy_pskills, give_class_kit, hp_fixed, fx_id);
	clif_displaymessage(fd, echo);
	...
	if (gid > 0) {
		...
		snprintf(buf, sizeof(buf),
			"[myclone] gid=%d dur=%dms friendly=%d copy_eq=%d skill=%d/%d biolab=%d fx=%d copy_pskills=%d kit=%d hp_fixed=%u extras=%d",
			gid, dur_ms, friendly, copy_eq, skill_id, skill_lv, biolab_i, fx_id, copy_pskills, give_class_kit, hp_fixed, fx_count);
		clif_displaymessage(fd, buf);
