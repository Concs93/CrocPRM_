...
static void clone_copy_player_skills(mob_skill_vec& dst, struct map_session_data* sd) {
    ...
    for (uint16 sk = 1; sk < MAX_SKILL; ++sk) {
        ...
        if (lv > 0)
            clone_add_mobskill(dst, sk, (uint16)lv, 1200, 0, 2000, 0);
    }
}
...
static void clone_give_class_kit(mob_skill_vec& dst, TBL_PC* sd)
{
                        ...
                        uint32 delay_ms, uint16 target_flags)
    {
        ...
        clone_add_mobskill(dst, id, lv, chance_permyriad, casttime_ms, delay_ms, target_flags);
    };
...
int mob_clone_spawn_ext(
    ...
    // 3) Modo/IA no DB (para o spawn já vir certo)
    db->status.mode = static_cast<e_mode>(mode);
#ifdef MD_AGGRESSIVE
    db->status.mode = static_cast<e_mode>(db->status.mode | MD_AGGRESSIVE);
#endif
#ifdef MD_CANMOVE
    db->status.mode = static_cast<e_mode>(db->status.mode | MD_CANMOVE);
#endif
#ifdef MD_CANATTACK
    db->status.mode = static_cast<e_mode>(db->status.mode | MD_CANATTACK);
#endif
#ifdef MD_CANUSESKILL
    db->status.mode = static_cast<e_mode>(db->status.mode | MD_CANUSESKILL);
#endif
    ...
    if ((flags & CLONE_ALLOW_EXTRA_SKILLS) && extra_skills && extra_count > 0) {
        for (int i = 0; i < extra_count; ++i) {
            ...
            clone_add_mobskill(db->skill, e.skill_id, e.skill_lv, e.chance_permyriad,
                               e.casttime_ms, e.delay_ms, e.target_flags);
        }
    }

	// 5) cria a instância
	TBL_MOB* md = mob_once_spawn_sub(nullptr, (int16)m, (int16)x, (int16)y,
									db->name.c_str(), mob_id,
									(death_event && *death_event) ? death_event : "",
